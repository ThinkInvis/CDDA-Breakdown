    ...
    if( has_effect( effect_in_pit ) ) {
        ...
        if (rng(0, 40) > get_str() + get_dex() / 2) {
            if(has_trait(trait_BD_DIGITIJUMP)) {
                add_msg_player_or_npc(m_good, _("You use your mutated legs to leap out of the pit!"),
...
void Character::reset_stats()
{
	...
	// BD: new bool flag over C:DDA master to figure out whether the player has a tail at all; TODO: is there a flag on the player for this? (mutcats or something)
    bool hasTail = false;
	...
	//BD: having weird backwards legs with nothing to help balance in exchange is Not Good
