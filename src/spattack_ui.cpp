...
//!!!!// Warning: lots of this is copied from mutation_ui.cpp; probably lots of work to do to make it usable
// '!' and '=' are uses as default bindings in the menu
const invlet_wrapper
spattack_chars("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ\"#&()*+./:;@[\\]^_{|}");
...
void show_spattacks_titlebar(const catacurses::window &window, std::string &menu_mode,
	input_context &ctxt)
{
	werase(window);
	...
	if (menu_mode == "reassigning") {
		desc << _("Reassigning.") << "  " <<
			_("Select a special attack to reassign or press <color_yellow>SPACE</color> to cancel. ");
	}
	if (menu_mode == "activating") {
		desc << "<color_green>" << _("Activating") << "</color>  " <<
			shortcut_desc(_("%s to examine special attack, "), ctxt.get_desc("TOGGLE_EXAMINE"));
	}
	if (menu_mode == "examining") {
		desc << "<color_light_blue>" << _("Examining") << "</color>  " <<
			shortcut_desc(_("%s to activate special attack, "), ctxt.get_desc("TOGGLE_EXAMINE"));
	}
	if (menu_mode != "reassigning") {
