...
void player::mend_item( item_location&& obj, bool interactive )
{
    ...
    if( interactive ) {
        ...
        for( auto& f : faults ) {
            ...
            descr << "> " << to_string_approx( time_duration::from_turns( f.first->time() / 100 * get_option<int>("VEHSTRUC_SCALING") / 100 ) ) << "\n";
    ...
    if( sel >= 0 ) {
        ...
        assign_activity( activity_id( "ACT_MEND_ITEM" ), faults[ sel ].first->time() * get_option<int>("VEHSTRUC_SCALING") / 100 );
